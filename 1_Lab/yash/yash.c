...
void parseString(char *str) {
    ...
    while ((token = strtok_r(cl_copy, " ", &save_ptr))) {
        ...
        if (recognizeCommand) {
            ...
            outRedirectIndices[cmdIndex] = numTokens + 1;
        } else if (strcmp(token, "<") == 0) {
            input[cmdIndex].hasInRedirect = true;
            inRedirectIndices[cmdIndex] = numTokens + 1;
        } else if (strcmp(token, "2>") == 0) {
            input[cmdIndex].hasErrorRedirect = true;
            errorRedirectIndices[cmdIndex] = numTokens + 1;
        } else if (strcmp(token, "|") == 0) {
...
void bg() {
    ...
    int jobsIdx;
    for (jobsIdx = 0; jobsIdx < 20; jobsIdx++) {
        if (g_jobs[jobsIdx].runOnForeBackground && g_jobs[jobsIdx].runInBackground == false) {
            if (jobsIdx > 0) {
                if (DEBUG)
                    printf("%s:%d g_jobs[%d] = %s\n", __FUNCTION__, __LINE__, jobsIdx,
                           g_jobs[jobsIdx].fullInput);
                g_jobs[jobsIdx-1].runOnForeBackground = false;
            }
            g_jobsCurrent = jobsIdx;
            g_jobs[jobsIdx].tstopped = false;
            g_currentPgid = g_jobs[jobsIdx].pgid;
...
void updateJobs() {
    ...
    // if the job is already running in the background, we set up the next runOnForeBackground
    if (g_jobs[g_jobsCurrent].runInBackground == true)
    {
        int i = g_jobsCurrent;
        while (g_jobs[i].runInBackground == true && i > 0)
        {
            g_jobs[i].runOnForeBackground = false;
            i--;
        }
        if (g_jobs[i].runInBackground == false)
            g_jobs[i].runOnForeBackground = true;
        if (DEBUG)
            printf("%s:%d new runOnForeBackground job is jobs[%d] = %s\n",
                   __FUNCTION__ , __LINE__, i, g_jobs[i].fullInput);
    }
    else {
        for (int i = 0; i < 20; i++) {
            g_jobs[i].runOnForeBackground = false;
