...
int main(void) {
  ...
  if (pid_ch1 > 0){
	...
	pid = waitpid(-1*pid_ch1, &status, WUNTRACED | WNOHANG);
  ...
  if (WIFSTOPPED(status)) {
   ...
   if (WIFCONTINUED(status)) {
      ...
      char *myargs[3];
      myargs[0] = strdup("sleep");   // program: "grep" (word count)
      myargs[1] = strdup("3");   // argument: "firefox"
      myargs[2] = NULL;           // marks end of array
      execvp(myargs[0], myargs);  // runs word count
    }
  } else {
      // Child 1
    setpgid(0, 0); // child 1 creates a new session and a new group and becomes leader -
